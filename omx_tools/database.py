...
def cmd_keyword(args):
    ...
    if not keyword:
        ...
        print("       omx-db keyword --json <keyword>")
        return
    if use_json:
        ...
        if schema_path.exists():
            ...
            if keyword in schema:
                ...
                return
            matches = {k: v for k, v in schema.items() if keyword.lower() in k.lower()}
            if matches:
                print(json.dumps(matches, indent=2, ensure_ascii=False))
                return
            print(f'Keyword "{keyword}" not found in schema. Falling back to DB search...\n')
        else:
            print(f"Schema file not found. Falling back to DB search...\n")
    db = get_db()
    rows = db.execute("""
