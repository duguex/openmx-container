...
File_Name=$(echo "$outputfile" |  cut -d "." -f 1)   #file name gain
...
if [ $spinpolarization_swich = NC ] ; then    ##if 1                                                                          
 ...
 echo "scf.SpinPolarization is Non-colliner"

else ###else 1

 echo "scf.SpinPolarization is colliner"
...
echo  "Please wait. Generating .MXenergy file ..."
...
wait
paste -d ' ' TEST1_1 TEST2_1 TEST3_1 > TEST4_1
wait
j=`expr $i + 1`
...
paste -d ' ' TEST4_1 eigen2_1 > TEST5_1
...
wait
paste -d ' ' TEST1_2 TEST2_2 TEST3_2 > TEST4_2
...
paste -d ' ' TEST4_2 eigen2_2 > TEST5_2
...
wait
paste -d ' ' TEST1_3 TEST2_3 TEST3_3 > TEST4_3
...
paste -d ' ' TEST4_3 eigen2_3 > TEST5_3
...
wait
paste -d ' ' TEST1_4 TEST2_4 TEST3_4 > TEST4_4
...
paste -d ' ' TEST4_4 eigen2_4 > TEST5_4
...
wait
paste -d ' ' TEST1_5 TEST2_5 TEST3_5 > TEST4_5
...
paste -d ' ' TEST4_5 eigen2_5 > TEST5_5
...
wait
paste -d ' ' TEST1_6 TEST2_6 TEST3_6 > TEST4_6
...
paste -d ' ' TEST4_6 eigen2_6 > TEST5_6
...
wait
paste -d ' ' TEST1_7 TEST2_7 TEST3_7 > TEST4_7
...
paste -d ' ' TEST4_7 eigen2_7 > TEST5_7
...
wait
paste -d ' ' TEST1_8 TEST2_8 TEST3_8 > TEST4_8
...
paste -d ' ' TEST4_8 eigen2_8 > TEST5_8
...
echo -e ".MXenergy file for BoltzTraP2 has been generated.\n"
...
echo "1" >> $File_Name.struct
echo "1 0 0 0 1 0 0 0 1" >> $File_Name.struct

sed -i '1s/^/Structure file of BoltzTrap for OpenMX\n/' $File_Name.struct

echo -e ".MXstructure file for BoltzTraP2 has been generated.\n"
...
###Up spin####
 echo  "Please wait. Generating .energyup file ..."
...
echo "Colliner::kloop kx ky kz:$i/$Parallel_1" 
...
echo "Colliner::kloop kx ky kz:$i2/$Parallel_2" 
...
echo "Colliner::kloop kx ky kz:$i3/$Parallel_3" 
...
echo "Colliner::kloop kx ky kz:$i4/$Parallel_4" 
...
echo "Colliner::kloop kx ky kz:$i5/$Parallel_5" 
...
echo "Colliner::kloop kx ky kz:$i6/$Parallel_6" 
...
echo "Colliner::kloop kx ky kz:$i7/$Parallel_7" 
...
wait 
echo "Colliner::kloop kx ky kz:$i8/$kloooopmax" 
...
wait
paste -d ' ' TEST1 TEST2 TEST3 > TEST4
...
sort -n test3 > test4
cat test4 | wc > eigen
cat eigen | sed 's/[\t ]\+/\t/g' | cut -f2 > eigen2
paste -d ' ' TEST4 eigen2 > TEST5
...
echo -e ".energy_upspin+downspin file for BoltzTraP2 has been generated.\n"
...
##END::  spinpolarization  on###
   ...
   ###added for BoltzTraP2
cd $Path/$File_Name
#pwd
#ls
wait
mkdir $Path/$File_Name/$File_Name'_BTP2'
wait
cp $outputfile $Path/$File_Name/$File_Name'_BTP2'
cp $File_Name.energy* $Path/$File_Name/$File_Name'_BTP2'/$File_Name.energy
cp $File_Name.struct $Path/$File_Name/$File_Name'_BTP2'/$File_Name.MXstructure
cp $File_Name.intrans $Path/$File_Name/$File_Name'_BTP2'
wait
cd $Path/$File_Name/$File_Name'_BTP2'

###structurefile###
grep "<Atoms.SpeciesAndCoordinates" -A 50000 $File_Name.out | grep "Atoms.SpeciesAndCoordinates>" -B 50000 | grep -v "<Atoms.SpeciesAndCoordinates" | grep -v "Atoms.SpeciesAndCoordinates>" | sed -e 's/^[ ]*//g' | sed 's/[\t ]\+/\t/g' | cut -f 2-5 | cat | expand -t 1 > atom_structure

wc atom_structure | sed 's/[\t ]\+/\t/g' | cut -f 2 | cat >  atom_structure_2

cat atom_structure_2 atom_structure > STU2

rm atom_structure
rm atom_structure_2
wait
head -n 4 $File_Name.MXstructure | cat > STU1
wait
cat STU1 STU2 >  $File_Name.MXstructure
rm STU1
rm STU2
###structurefile end###
###energyfile###
head -n 1 $File_Name.energy | cat > EN1
head -n 2 $File_Name.energy | tail -n 1 | cat > EN2_1
echo "1" | cat > EN2_2
cat $File_Name.intrans | head -n 3 | tail -n 1 | cat | sed -e 's/^[ ]*//g' | sed 's/[\t ]\+/\t/g' | cut -f 1 | cat > EN2_3
wait
spinpolarization_swich=$(grep scf.SpinPolarization $outputfile | sed -e 's/^[ ]*//g' | sed 's/[\t ]\+/\t/g' | cut -f 2 | sed -e 's/\(^.*\)/\U\1/' | cat ) 
    if [ $spinpolarization_swich = NC -o $spinpolarization_swich = nc -o $spinpolarization_swich = Nc -o $spinpolarization_swich = on -o $spinpolarization_swich = On -o $spinpolarization_swich = ON  ] ; then
    paste -d ' ' EN2_1 EN2_2 EN2_3 EN2_2 > EN2
    else
    echo "0" | cat > EN2_4
    paste -d ' ' EN2_1 EN2_2 EN2_3 EN2_4> EN2
    rm EN2_4
    fi


#paste -d ' ' EN2_1 EN2_2 EN2_3 > EN2
sed '1,2d' $File_Name.energy > EN3
wait
cat EN1 EN2 EN3 > $File_Name.MXenergy
wait
rm EN*
 
cd $Path/$File_Name
rm *.energy* *.struct *.out *.intrans
cd $Path/$File_Name/$File_Name'_BTP2'
cp * $Path/$File_Name
rm $Path/$File_Name/*.intrans

sed -i 's/BoltzTrap/BoltzTrap2/g' $Path/$File_Name/*.MXstructure
sed -i 's/BoltzTrap/BoltzTrap2/g' $Path/$File_Name/*.MXenergy 
rm -r $Path/$File_Name/$File_Name'_BTP2'
   ###
cd $Path/$File_Name
pwd
rm $Path/$File_Name/*.energy*
ls
exit 0
