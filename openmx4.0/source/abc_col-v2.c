  ...
  *************************************************************/
  ...
  for (i=0; i<No[spin]; i++){
    for (j=0; j<Nuo[spin]; j++){
      printf("%6.3f ",(eval[j]-eval[i+No[spin]])*(A12[0][0][j*No[spin]+i].r+A12[1][0][j*No[spin]+i].r));
    } 
    printf("\n");
  } 
  ...
  for (i=0; i<No[spin]; i++){
    for (j=0; j<Nuo[spin]; j++){
      printf("%6.3f ",(eval[j]-eval[i+No[spin]])*(A12[0][0][j*No[spin]+i].i+A12[1][0][j*No[spin]+i].i));
    } 
    printf("\n");
  } 
  ...
  for (i=0; i<No[spin]; i++){
    for (j=0; j<Nuo[spin]; j++){
      printf("%6.3f ",P[0][(j+No[spin])*MatDim+i].r);
    } 
    printf("\n");
  } 
  ...
  for (i=0; i<No[spin]; i++){
    for (j=0; j<Nuo[spin]; j++){
      printf("%6.3f ",P[0][(j+No[spin])*MatDim+i].i);
    } 
    printf("\n");
  } 
  ...
  for (xyz_i=0; xyz_i<3; xyz_i++){   
    for (m=0; m<MatDim; m++){ 
      ...
      for (n=0; n<MatDim; n++){
        ...
        ctmp1.r = dHk[xyz_i][m*MatDim+n].r - em*dSk[xyz_i][m*MatDim+n].r;
        ctmp1.i = dHk[xyz_i][m*MatDim+n].i - em*dSk[xyz_i][m*MatDim+n].i;
  ...
  *************************************************************/
  ...
  for (xyz_i=0; xyz_i<3; xyz_i++){   
	...
	tmp1 = d/(d*d + del);
        ...
        ctmp1.r = (dHk[xyz_i][m*MatDim+ln].r-em*dSk[xyz_i][m*MatDim+ln].r)*tmp1;  
        ctmp1.i = (dHk[xyz_i][m*MatDim+ln].i-em*dSk[xyz_i][m*MatDim+ln].i)*tmp1;

        /* multiplying the imaginary number, i */
	A21[1][xyz_i][m*Nuo[spin]+n].r =-ctmp1.i; 
	A21[1][xyz_i][m*Nuo[spin]+n].i = ctmp1.r; 
	...
	tmp1 = d/(d*d + del);
        ...
        ctmp1.r = (dHk[xyz_i][lm*MatDim+n].r-em*dSk[xyz_i][lm*MatDim+n].r)*tmp1;
        ctmp1.i = (dHk[xyz_i][lm*MatDim+n].i-em*dSk[xyz_i][lm*MatDim+n].i)*tmp1; 

        /* multiplying the imaginary number, i */
	A12[1][xyz_i][m*No[spin]+n].r =-ctmp1.i; 
	A12[1][xyz_i][m*No[spin]+n].i = ctmp1.r;
                   ...
                   double **PF1, dcomplex ***ABC11, double *eval )
{
  ...
  dcomplex ctmp1,ctmp2,csum,*Hk,*Sk,*WF;
	...
	for (LB_AN=0; LB_AN<=FNAN[GA_AN]; LB_AN++){
	  ...
	  for (i=0; i<tnoA; i++){
	    for (j=0; j<tnoB; j++){
	      ...
	      ABC11[0][0][n*No[spin]+m].r += ctmp2.r*(sx - 0.0*Rx*s); 
	      ABC11[0][1][n*No[spin]+m].r += ctmp2.r*(sy - 0.0*Ry*s); 
	      ABC11[0][2][n*No[spin]+m].r += ctmp2.r*(sz - 0.0*Rz*s); 

	      ABC11[0][0][n*No[spin]+m].i += ctmp2.i*(sx - 0.0*Rx*s); 
	      ABC11[0][1][n*No[spin]+m].i += ctmp2.i*(sy - 0.0*Ry*s); 
	      ABC11[0][2][n*No[spin]+m].i += ctmp2.i*(sz - 0.0*Rz*s); 
  ...
  *************************************************************/
  ...
  /*
  printf("Fk1.r k1=%8.5f k2=%8.5f k3=%8.5f\n",k1,k2,k3); 
  for (i=0; i<MatDim; i++){ 
    for (j=0; j<No[spin]; j++){ 
      printf("%6.3f ",Fk1[j*MatDim+i].r); 
    }
    printf("\n");
  }

  printf("Fk1.i k1=%8.5f k2=%8.5f k3=%8.5f\n",k1,k2,k3); 
  for (i=0; i<MatDim; i++){ 
    for (j=0; j<No[spin]; j++){ 
      printf("%6.3f ",Fk1[j*MatDim+i].i); 
    }
    printf("\n");
  }
  */
  ...
  /*
  printf("B11.r k1=%8.5f k2=%8.5f k3=%8.5f\n",k1,k2,k3); 
  for (i=0; i<No[spin]; i++){ 
    for (j=0; j<No[spin]; j++){ 
      printf("%6.3f ",B11[i*No[spin]+j].r); 
    }
    printf("\n");
  }

  printf("B11.i k1=%8.5f k2=%8.5f k3=%8.5f\n",k1,k2,k3); 
  for (i=0; i<No[spin]; i++){ 
    for (j=0; j<No[spin]; j++){ 
      printf("%6.3f ",-B11[i*No[spin]+j].i); 
    }
    printf("\n");
  }
  */
  ...
  for (p=0; p<3; p++){
    ...
    m = No[spin]; n = No[spin]; k = MatDim;
    zgemm_("N","N", &m, &n, &k, &alpha, Hk, &m, Sk, &k, &beta, ABC11[1][p], &m);

    for (i=0; i<No[spin]*No[spin]; i++){ 
      ABC11[1][p][i].r = ABC11[1][p][i].i; 
      ABC11[1][p][i].i =-ABC11[1][p][i].r; 
    }
  }
