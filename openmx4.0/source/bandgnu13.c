...
int main(int argc, char **argv)
{
  ...
  char fname[800],fnamedat1[3][800],*pos_dot,ticsname[800];
  ...
  for (spin=0;spin<= mspin;spin++) {
    ...
    strncpy(fnamedat1[spin], argv[1], sizeof(fnamedat1[spin])-1);
    fnamedat1[spin][sizeof(fnamedat1[spin])-1] = '\0';

    pos_dot = strrchr(fnamedat1[spin], '.');
    if (pos_dot) *pos_dot = '\0';

    {
      size_t len = strlen(fnamedat1[spin]);
      int n = snprintf(fnamedat1[spin] + len,
		       sizeof(fnamedat1[spin]) - len,
		       ".BANDDAT%d", spin+1);

      if (n < 0 || (size_t)n >= sizeof(fnamedat1[spin]) - len) {
  ...
  pos_dot = strrchr(fname, '.');
  if (pos_dot) *pos_dot = '\0';

  {
    size_t len = strlen(fname);
    const char *suffix = ".GNUBAND";

    if (len + strlen(suffix) + 1 > sizeof(fname)) {
      fprintf(stderr, "file name is too long: %s\n", argv[1]);
      exit(10);
    }
