...
!    print *, "from elpa_solve_evp_complex_2stage_double_impl.F90"
    ...
    if (do_bandred) then
     ...
     if (do_tridiag) then
       ...
       if (istat .ne. 0) then
         ...
         &MATH_DATATYPE&
         &_2stage_&
         &ELPA_PRECISION " // ": error when allocating e "//errorMessage
         stop 1
       endif
...
!       do_useGPU_tridiag_band, wantDebug, nrThreads)
     ...
     if (do_trans_to_band) then
       ...
       if (istat .ne. 0) then
         ...
         &MATH_DATATYPE&
         &_2stage_&
         &ELPA_PRECISION " // ": error when deallocating hh_trans "//errorMessage
         stop 1
       endif
     ...
     if (do_trans_to_full) then
       ...
       if (istat .ne. 0) then
         ...
         &MATH_DATATYPE&
         &_2stage_&
         &ELPA_PRECISION " // ": error when deallocating tmat"//errorMessage
         stop 1
       endif
