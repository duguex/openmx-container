...
void SetPara_DFT()
{
  ...
  ****************************************************/
  ...
  /* Added by S.Ryee */
  if (MYID_MPI_COMM_WORLD==Host_ID && 0<level_stdout && Hub_U_switch==1){
    printf("\n");
    printf("*******************************************************\n"); fflush(stdout);
  ...
  ***************************************************/
  ...
  if ( CWF_Calc==1 && CWF_Guiding_Orbital==4 ){
  ...
  *************************/
  ...
  for (spe=0; spe<real_SpeciesNum; spe++){
    ...
    if ((fp = fopen(FN_VPS,"r")) != NULL){
      ...
      if (VPS_j_dependency[spe]==0 && MYID_MPI_COMM_WORLD==Host_ID && 0<level_stdout) {
        printf("               %s.vps is l-dependent.\n",SpeVPS[spe]);
      }
      else if (SO_switch==1 && VPS_j_dependency[spe]==1 && MYID_MPI_COMM_WORLD==Host_ID && 0<level_stdout) {
...
void Read_VPS(int spe, char *file)
{
  ...
  ****************************************************/
  ...
  if (Number_of_Electrons_Species_flag==1){

    if (Spe_Core_Charge[spe]!=0.0)
      charge_factor = Number_of_Electrons_Species[spe]/Spe_Core_Charge[spe];
    else 
      charge_factor = 1.0;

    Spe_Core_Charge[spe] = Number_of_Electrons_Species[spe]; 
  }
