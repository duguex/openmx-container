             ...
             int *WhatSpecies_NEB, int *Spe_WhatAtom_NEB, char **SpeName_NEB) 
{
  ...
  ****************************************************/
  ...
  if (index_images==8){
    int ct_AN = 1;
    int ID = G2ID[ct_AN];
    printf("FFF2 myid=%2d index_images=%2d ct_AN=%2d ID=%2d\n",
            myid,index_images,ct_AN,ID);

    MPI_Finalize();
    exit(0);
  }  

  if (index_images==7 || index_images==8){
  ...
  */
  ...
  if (dftD_switch==1 && version_dftD==2) DFTDvdW_init(1);
  if (dftD_switch==1 && version_dftD==3) DFTD3vdW_init(1);
