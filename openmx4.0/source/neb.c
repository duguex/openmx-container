...
void Make_XYZ_File(char fname[YOUSO10], double ***neb_atom_coordinates)
{
  ...
  if (myid==Host_ID){
	...
	fprintf(fp,"%i \n",atomnum);
	fprintf(fp,"  Image index = %3d  Energy= %8.5f (Hartree)\n",p,neb_atom_coordinates[p][0][0]);
...
void Make_MD_File(char fname[YOUSO10], double ***neb_atom_coordinates)
{
  ...
  if (myid==Host_ID){
	...
	fprintf(fp,"%i \n",atomnum);
	fprintf(fp,"  Image index = %3d  Energy= %8.5f (Hartree) ",p,neb_atom_coordinates[p][0][0]);
...
void generate_input_files(char *file, int iter)
{
  ...
  if (myid==Host_ID){
	...
	while (fgets(st,800,fp2)!=NULL){
	  ...
	  else if (strncmp(st1,"level.of.stdout",15)==0){

            int flag=-1;

            if (numprocs==1){ flag = 1; }
            else if (PN==0){
              if (p<=(NEB_Num_Images/numprocs)) flag = 1; 
              if (p==1) flag = 1;
	    }
            else if (PN!=0){
              if (p<=(NEB_Num_Images/PN)) flag = 1; 
	    }

	    fprintf(fp1,"level.of.stdout                   %d\n",flag);
	    level_flag = 1;
	  } 
  ...
  if (Scf_RestartFromFile){
	...
	/*  FRAC to AU */
	    ...
	    for (k=1; k<=3; k++){
	      ...
	      itmp = (int)neb_atom_coordinates[p][i][k]; 

              if (1.0<neb_atom_coordinates[p][i][k]){

                neb_atom_coordinates[p][i][k] = neb_atom_coordinates[p][i][k] - (double)itmp;

		if (myid==Host_ID){
		  if (k==1) printf("The fractional coordinate of a-axis for atom %d of the %d-th image was translated within the range (0 to 1).\n",i,p);
		  if (k==2) printf("The fractional coordinate of b-axis for atom %d of the %d-th image was translated within the range (0 to 1).\n",i,p);
		  if (k==3) printf("The fractional coordinate of c-axis for atom %d of the %d-th image was translated within the range (0 to 1).\n",i,p);
		}
	      }

              else if (neb_atom_coordinates[p][i][k]<0.0){

                neb_atom_coordinates[p][i][k] = neb_atom_coordinates[p][i][k] + (double)(abs(itmp)+1);
	  ...
	  for (i=1; i<=atomnum; i++){
	    ...
	    x = neb_atom_coordinates[p][i][1]*tv[1][1]
	      + neb_atom_coordinates[p][i][2]*tv[2][1] 
	      + neb_atom_coordinates[p][i][3]*tv[3][1] + tmpx;

	    y = neb_atom_coordinates[p][i][1]*tv[1][2] 
	      + neb_atom_coordinates[p][i][2]*tv[2][2] 
	      + neb_atom_coordinates[p][i][3]*tv[3][2] + tmpy;

	    z = neb_atom_coordinates[p][i][1]*tv[1][3]
	      + neb_atom_coordinates[p][i][2]*tv[2][3]
	      + neb_atom_coordinates[p][i][3]*tv[3][3] + tmpz;

	    neb_atom_coordinates[p][i][1] = x;
	    neb_atom_coordinates[p][i][2] = y;
	    neb_atom_coordinates[p][i][3] = z;
	  }
