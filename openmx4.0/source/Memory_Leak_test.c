...
void Get_VSZ2(int MD_iter)
{
  ...
  sprintf(fname0,"ps_result_%d",MD_iter);
  sprintf(fname1,"ls_dat000003");
  sprintf(fname2,"ls_dat000004");
  ...
  if ( (fp = fopen(fname0,"r")) != NULL ){
    ...
    num = 0;
    po_VSZ = 0; column_VSZ = 0;
    po_RSS = 0; column_RSS = 0; 
    po_CPU = 0; column_CPU = 0;
  ...
  /* find the lines which corresponds to the OpenMX job */
  ...
  if ( (fp = fopen(fname0,"r")) != NULL ){
    ...
    do {
      ...
      if (fgets(tmpline, 300, fp)!=NULL){
	    ...
	    if      ( (tp != NULL) && num==column_CPU ) Used_CPU += atof(tp);
	    else if ( (tp != NULL) && num==column_VSZ ) Used_VSZ += atof(tp);
	    else if ( (tp != NULL) && num==column_RSS ) Used_RSS += atof(tp);
  ...
  printf("Get_VSZ2: MD_iter= %2d %15.10f %15.10f %15.10f\n",MD_iter,Used_CPU,Used_VSZ,Used_RSS);
