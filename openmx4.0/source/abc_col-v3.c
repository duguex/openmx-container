...
dcomplex rval(int n, int m, dcomplex **Afull, dcomplex r[3]);

dcomplex rval_gg( int n, int m, double *eval, double **omega_gg, 
		  dcomplex **Afull, dcomplex r_gg[3][3] );
...
void SHG()
{
  ...
  double k1,k2,k3,tmp1,tmp2,omega,omega_min,omega_max,domega,eta;
  double fl,fn,fm,en,em,el,x,max_x=30.0,Beta,wmn,wml,wln,fln,fnm,fml;
  double *eval,**omega_gg,sum0,sum1,sum2,w1,w2,w3,w4,Dmnb,Dmnc;
  dcomplex cw1,cw2,cw3,cw4,cw;
  dcomplex rnm[3],rml[3],rln[3],rmn[3],ctmp1,ctmp2,ctmp3,ctmp4;
  dcomplex rnm_gg[3][3],rmn_gg[3][3],term1,term2,term3,term4;
  dcomplex **Afull,***A11,***A12,***A21,***A22,*Hk,*Sk,*WF,**dHk,**dSk;
  ...
  *************************************************************/
  ...
  for (spin=0; spin<=SpinP_switch; spin++){ 
    ...
    for (kloop=0; kloop<T_knum; kloop++){
      ...
      /*
      for (p=0; p<3; p++){ 
        printf("A11.r kloop=%2d p=%2d\n",kloop,p); 
	for (i=0; i<No[spin]; i++){
	  for (j=0; j<No[spin]; j++){
	    printf("%6.3f ",A11[0][p][j*No[spin]+i].r+A11[1][p][j*No[spin]+i].r);
	  }
          printf("\n");
	}
      }
      ...
      for (p=0; p<3; p++){ 
        printf("A11.i kloop=%2d p=%2d\n",kloop,p); 
	for (i=0; i<No[spin]; i++){
	  for (j=0; j<No[spin]; j++){
	    printf("%6.3f ",A11[0][p][j*No[spin]+i].i+A11[1][p][j*No[spin]+i].i);
	  }
          printf("\n");
	}
      }
      ...
      for (p=0; p<3; p++){ 
        printf("A12.r kloop=%2d p=%2d\n",kloop,p); 
	for (i=0; i<No[spin]; i++){
	  for (j=0; j<Nuo[spin]; j++){
	    printf("%6.3f ",A12[0][p][j*No[spin]+i].r+A12[1][p][j*No[spin]+i].r);
	  }
          printf("\n");
	}
      }
      ...
      for (p=0; p<3; p++){ 
        printf("A12.i kloop=%2d p=%2d\n",kloop,p); 
	for (i=0; i<No[spin]; i++){
	  for (j=0; j<Nuo[spin]; j++){
	    printf("%6.3f ",A12[0][p][j*No[spin]+i].i+A12[1][p][j*No[spin]+i].i);
	  }
          printf("\n");
	}
      }
      ...
      for (p=0; p<3; p++){ 
        printf("A21.r kloop=%2d p=%2d\n",kloop,p); 
	for (i=0; i<Nuo[spin]; i++){
	  for (j=0; j<No[spin]; j++){
	    printf("%6.3f ",A21[0][p][j*Nuo[spin]+i].r+A21[1][p][j*Nuo[spin]+i].r);
	  }
          printf("\n");
	}
      }
      ...
      for (p=0; p<3; p++){ 
        printf("A21.i kloop=%2d p=%2d\n",kloop,p); 
	for (i=0; i<Nuo[spin]; i++){
	  for (j=0; j<No[spin]; j++){
	    printf("%6.3f ",A21[0][p][j*Nuo[spin]+i].i+A21[1][p][j*Nuo[spin]+i].i);
	  }
          printf("\n");
	}
      }
      ...
      for (p=0; p<3; p++){ 
        printf("A22.r kloop=%2d p=%2d\n",kloop,p); 
	for (i=0; i<Nuo[spin]; i++){
	  for (j=0; j<Nuo[spin]; j++){
	    printf("%6.3f ",A22[0][p][j*Nuo[spin]+i].r+A22[1][p][j*Nuo[spin]+i].r);
	  }
          printf("\n");
	}
      }
      ...
      for (p=0; p<3; p++){ 
        printf("A22.i kloop=%2d p=%2d\n",kloop,p); 
	for (i=0; i<Nuo[spin]; i++){
	  for (j=0; j<Nuo[spin]; j++){
	    printf("%6.3f ",A22[0][p][j*Nuo[spin]+i].i+A22[1][p][j*Nuo[spin]+i].i);
	  }
          printf("\n");
	}
      }
		...
		if (m!=l && n!=l){
		  ...
		  */
		  ...
		  tmp1 = (2.0*fnm/(wmn-2.0*omega) + fln/(wln-omega) + fml/(wml-omega))/(wln - wml);
                  ...
                  //printf("ABC1 kloop=%2d p=%2d n=%2d m=%2d l=%2d %15.12f %15.12f %15.12f\n",
                  //       kloop,p,n,m,l,cw.r,cw.i,tmp1);

		  rval(n, m, Afull, rnm);
		  rval(m, l, Afull, rml);
		  rval(l, n, Afull, rln);

		  //                  printf("ABC1 kloop=%2d p=%2d n=%2d m=%2d l=%2d %5.3f %5.3f %5.3f %5.3f %5.3f %5.3f %5.3f %5.3f %5.3f\n",kloop,p,n,m,l,rnm[0].r,rnm[1].r,rnm[2].r,rml[0].r,rml[1].r,rml[2].r,rln[0].r,rln[1].r,rln[2].r);
		  ...
		  for (a=0; a<3; a++){
		    for (b=0; b<3; b++){
		      for (c=0; c<3; c++){
                        ...
                        //sprintf(nanchar1,"%8.4f",ctmp4.r);
                        //sprintf(nanchar2,"%8.4f",ctmp4.i);

                        //chi0[a][b][c].r += ctmp4.r*cw.r - ctmp4.i*cw.i;
                        //chi0[a][b][c].i += ctmp4.r*cw.i + ctmp4.i+cw.r;

                        chi0[a][b][c].r += ctmp4.r*tmp1;
                        chi0[a][b][c].i += ctmp4.i*tmp1;
                                 
	   	        //printf("ABC1 kloop=%2d p=%2d n=%2d m=%2d l=%2d a=%2d b=%2d c=%2d %5.3f %5.3f %5.3f\n",
			//       kloop,p,n,m,l,a,b,c,ctmp4.r,ctmp4.i,tmp1);
	...
	for (a=0; a<3; a++){
	  for (b=0; b<3; b++){
	    for (c=0; c<3; c++){
	      printf("VVV1 kloop=%2d p=%2d a=%2d b=%2d c=%2d %15.12f %15.12f\n",
                     kloop,p,a,b,c,chi0[a][b][c].r,chi0[a][b][c].i);
	    }
	  }
	}
	*/
	...
	for (a=0; a<3; a++){
	  for (b=0; b<3; b++){
	    for (c=0; c<3; c++){
	      ...
	      chi_e[a][b][c][p].r += 0.5*chi0[a][b][c].r;
	      chi_e[a][b][c][p].i += 0.5*chi0[a][b][c].i;
	    }
	  }
	}
	  ...
	  for (m=0; m<MatDim; m++){
	    ...
	    if (n!=m){
	      ...
	      rval(m, n, Afull, rmn);
              ...
              rval_gg(n, m, eval, omega_gg, Afull, rnm_gg);              
              rval_gg(m, n, eval, omega_gg, Afull, rmn_gg);              
		...
		for (b=0; b<3; b++){
		  for (c=0; c<3; c++){
		    ...
		    chi0[a][b][c].r += fnm*(term1.r + term2.r + term3.r + term4.r);
		    chi0[a][b][c].i += fnm*(term1.i + term2.i + term3.i + term4.i);
	...
	for (a=0; a<3; a++){
	  for (b=0; b<3; b++){
	    for (c=0; c<3; c++){
              /* multiply 0.5i and add it to chi_e */  
	      chi_i[a][b][c][p].r +=-0.5*chi0[a][b][c].i;
	      chi_i[a][b][c][p].i += 0.5*chi0[a][b][c].r;
	    } // c
	  } // b
	} // a
  ...
  for (p=0; p<Ng; p++){
    ...
    omega = omega_min + (double)p*domega;
    printf("%15.12f %15.12f %15.12f\n",omega,chi_e[0][1][2][p].i,chi_i[0][1][2][p].r);
  }
...
dcomplex rval(int n, int m, dcomplex **Afull, dcomplex r[3])
{
  r[0].r = Afull[0][m*MatDim+n].r;
  r[1].r = Afull[1][m*MatDim+n].r;
  r[2].r = Afull[2][m*MatDim+n].r;
  r[0].i = Afull[0][m*MatDim+n].i;
  r[1].i = Afull[1][m*MatDim+n].i;
  r[2].i = Afull[2][m*MatDim+n].i;
}
	    ...
	    for (j=0; j<tnoB; j++){
	      ...
	      /* store the result to A11, A12, A21, or A22 */
              ...
              if (m<No[spin] && n<No[spin]){
                A11[0][0][n*No[spin]+m].r += ctmp2.r*(sx - 0.0*Rx*s);
                A11[0][1][n*No[spin]+m].r += ctmp2.r*(sy - 0.0*Ry*s);
                A11[0][2][n*No[spin]+m].r += ctmp2.r*(sz - 0.0*Rz*s);

                A11[0][0][n*No[spin]+m].i += ctmp2.i*(sx - 0.0*Rx*s);
                A11[0][1][n*No[spin]+m].i += ctmp2.i*(sy - 0.0*Ry*s);
                A11[0][2][n*No[spin]+m].i += ctmp2.i*(sz - 0.0*Rz*s);
	      }
                ...
                A22[0][0][(n-No[spin])*Nuo[spin]+m-No[spin]].r += ctmp2.r*sx;
                A22[0][1][(n-No[spin])*Nuo[spin]+m-No[spin]].r += ctmp2.r*sy;
                A22[0][2][(n-No[spin])*Nuo[spin]+m-No[spin]].r += ctmp2.r*sz;

                A22[0][0][(n-No[spin])*Nuo[spin]+m-No[spin]].i += ctmp2.i*sx;
                A22[0][1][(n-No[spin])*Nuo[spin]+m-No[spin]].i += ctmp2.i*sy;
                A22[0][2][(n-No[spin])*Nuo[spin]+m-No[spin]].i += ctmp2.i*sz;
	      }
