...
void EigenValue_Problem(double k1,double k2,double k3, int calc_switch){
  ...
  int fsize=n;
  int fsize2=2*fsize;
  dcomplex *matrix1=(dcomplex*)malloc(sizeof(dcomplex)*fsize2*fsize2);
  dcomplex *matrix2=(dcomplex*)malloc(sizeof(dcomplex)*fsize2*fsize2);
  dcomplex *matrix3=(dcomplex*)malloc(sizeof(dcomplex)*fsize2*fsize2);
  double *rwork=(double*)malloc(sizeof(double)*(3*fsize2-2));
  int itype=1;
  int lwork=-1;
  int info;
  dcomplex opt;
  zhegv_(&itype, "V", "U", &fsize2, matrix1, &fsize2, matrix2, &fsize2, ko+1, &opt, &lwork, rwork, &info);
  lwork=opt.r;
  ...
  */
    ...
    */
    ...
    zhegv_(&itype, "V", "U", &fsize2, matrix1, &fsize2, matrix2, &fsize2, ko+1, work, &lwork, rwork, &info);
    for(l=1; l<=2*n; l++){
      ko1=(ko[l]-ChemP)*eV2Hartree;
      EIGEN[l]=ko1;
    }
    if(calc_switch==1){
      for (i1=1; i1<=fsize; i1++){
      ...
      dcomplex alpha={1.0, 0.0}, beta=zero;
      zgemm_("N", "N", &fsize2, &l_max, &fsize2, &alpha, matrix2, &fsize2, matrix1, &fsize2, &beta, matrix3, &fsize2);
      for (l=l_min; l<=l_max; l++){
        for (j1=1; j1<=fsize; j1++){
          dcomplex v1a=matrix1[(j1-1)+(l-1)*fsize2];
          dcomplex v2a=matrix3[(j1-1)+(l-1)*fsize2];
          dcomplex v1b=matrix1[(j1-1+fsize)+(l-1)*fsize2];
          dcomplex v2b=matrix3[(j1-1+fsize)+(l-1)*fsize2];
          C[l][j1].r=v1a.r*v2a.r+v1a.i*v2a.i;
          C[l+fsize][j1+fsize].r=v1b.r*v2b.r+v1b.i*v2b.i;
          C[l][j1+fsize].r=v1a.r*v2b.r+v1a.i*v2b.i;
          C[l][j1+fsize].i=v1a.r*v2b.i-v1a.i*v2b.r;
        }
      }
    }
  /*
    ...
    for (GA_AN=1; GA_AN<=atomnum; GA_AN++){
      ...
      for (i1=0; i1<tnoA; i1++){
        ...
        for(l=l_min; l<=l_max; l++){
          ...
          if (mode==(int(*)())FermiLoop){
            if (l==Nband[0]){
              Data_MulP[0][0][GA_AN][i1]=C[l][ia].r;
              Data_MulP[1][0][GA_AN][i1]=C[l+fsize][ia+fsize].r;
              Data_MulP[2][0][GA_AN][i1]=C[l][ia+fsize].r;
              Data_MulP[3][0][GA_AN][i1]=-C[l][ia+fsize].i;
            }
          } else {
#endif
            Data_MulP[0][l-l_min][GA_AN][i1]=C[l][ia].r;
            Data_MulP[1][l-l_min][GA_AN][i1]=C[l+fsize][ia+fsize].r;
            Data_MulP[2][l-l_min][GA_AN][i1]=C[l][ia+fsize].r;
            Data_MulP[3][l-l_min][GA_AN][i1]=-C[l][ia+fsize].i;
#ifndef DEBUG_SIGMAEK_OLDER_20181126
	  }
#endif
	}//l
