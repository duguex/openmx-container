...
void Allocation_Atoms_3D(int MD_iter, int weight_flag)
{
    ...
    for (child=0; child<n; child++){
      ...
      for (i=1; i<=3; i++){
	    ...
	    sum += weight[m]*(Gxyz[m][i]-xyz_c[i])*(Gxyz[m][j]-xyz_c[j]);
          ...
          /* intentionally break dengeracay if there is */
          IMT[i][j] = sum - (1.0e-7)*cos((double)(i*j)) + (1.0e-7)*sin((double)(i+j));
        }
      }
  ...
  */
  ...
  for (child=0; child<n; child++){
    ...
    qsort_int1((long)Na,List_AN_in_Child[max_depth][child]); 
  }  
  ...
  *********************************************/
  ...
  if (MYID_MPI_COMM_WORLD==Host_ID && 1<=MD_iter && 0<level_stdout){
    printf("\n");
    printf("*******************************************************\n"); 
  ...
  if (MYID_MPI_COMM_WORLD==Host_ID && 1<=MD_iter && 0<level_stdout) printf("\n\n\n");
