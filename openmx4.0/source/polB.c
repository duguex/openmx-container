...
int main(int argc, char *argv[]) 
{
  ...
  ******************************************/
  if      (SpinP_switch==0){ spinsize=1; fsize2=fsize;  fsize3=fsize+2; fsize4=Valence_Electrons/2;}
    ...
    fsize4=Valence_Electrons/2+abs((int)floor(Total_SpinS))*2+1;}
  ...
  *************************************************/
  ...
  if (myid==Host_ID){
    ...
    */
    ...
    /* Added by N. Yamaguchi ***/
    printf("\n********************************************************************\n"); fflush(stdout);
    printf("  Note: Bulk electric polarization has arbitrariness of eRi/Omega.    \n"); fflush(stdout);
    printf("********************************************************************\n\n"); fflush(stdout);

    printf(" (muC/cm^2)         R1                R2                R3\n"); fflush(stdout);
    double Parbs[3][3];
    for (i=0; i<3; i++){
      for (j=0; j<3; j++){
    ...
    for (spin = 0; spin < spinsize; spin++) {
      ...
      for (mu1 = 0; mu1 < numBand; mu1++) {
        for (mu2 = 0; mu2 < numBand; mu2++) {
          Sop[spin][mu1+1][mu2+1] = matrix2[mu1 + mu2 * numBand];
        }
      }
      if (SpinP_switch == 3) {
        for (mu1 = 0; mu1 < numBand; mu1++) {
        ...
        for (mu1 = 0; mu1 < numBand; mu1++) {
          for (mu2 = 0; mu2 < numBand; mu2++) {
            matrix2[mu1 + mu2 * numBand] = Sop[spin][mu1+1][mu2+1];
          }
        }
        beta.r = 1.0;
        zgemm_("N", "N", &numBand, &numBand, &fsize, &alpha, matrix3, &numBand,
