...
void InitV()
{
  ...
  ********************************************************/
  ...
  double ax,ay,az,tmp,Wscale,sum,v,sumPx,sumPy,sumPz,scale;
  ...
  if ( ( MD_switch==2  || // NVT_VS
	...
	Gxyz[Gc_AN][26] = v*az;
        ...
        sumPx += Gxyz[Gc_AN][24]*Gxyz[Gc_AN][20];
        sumPy += Gxyz[Gc_AN][25]*Gxyz[Gc_AN][20];
        sumPz += Gxyz[Gc_AN][26]*Gxyz[Gc_AN][20];
      }

      /* correction so that the sum of momenta can be zero.  */

      sumPx /= (double)atomnum; 
      sumPy /= (double)atomnum; 
      sumPz /= (double)atomnum; 

      for (Gc_AN=1; Gc_AN<=atomnum; Gc_AN++){
	Gxyz[Gc_AN][24] -= sumPx/Gxyz[Gc_AN][20];
	Gxyz[Gc_AN][25] -= sumPy/Gxyz[Gc_AN][20];
	Gxyz[Gc_AN][26] -= sumPz/Gxyz[Gc_AN][20];
