...
static int tran_bias_apply;
static double tran_biasvoltage_e[2];

static char filepath[100];
static char filename[100];
static char tranbfilename[100];
                         ...
                         int TNumGrid )
{
  ...
  **********************************************/
  ...
  if (myid0==Host_ID){  
    ...
    printf(" Copyright (C), 2002-2026, H. Kino and T. Ozaki        \n"); 
  ...
  **********************************************/
  ...
  MTRAN_Input_Sys(argc,argv[1],filepath,filename,&tran_interpolate,interpolate_filename1,interpolate_filename2,tranbfilename);

  /**********************************************
                 read tranb file
  **********************************************/

  MTRAN_Read_Tran_HS(comm1, filepath, tranbfilename, "tranb" );
  ...
  if (tran_transmission_on) {
    ...
    ***********************************************************/
    ...
    for (kloop0=0; kloop0<num_kloop0; kloop0++){
      ...
      printf("  myid0=%5d num_kloop0=%2d kloop0=%2d i2=%2d i3=%2d  k2=%8.4f k3=%8.4f\n",
                num_kloop0,kloop0,myid0,i2,i3,k2,k3); fflush(stdout);
      ...
      MTRAN_Set_SurfOverlap( "left", k2,k3,SpinP_switch,atomnum_e,
			     ...
			     FNAN_e,natn_e,ncn_e,atv_ijk_e,S00_e,S01_e,H00_e,H01_e,
                             tran_bias_apply, tran_biasvoltage_e);
    ...
    for (k = 0; k < SpinP_switch + 1; k++){
      for (iside = 0; iside < 3; iside++){
        for (i = 0; i < NUM_c; i++){
          ...
          for (j = 0; j < NUM_c; j++){
            JLocSym[k][iside][i][j] = JLocSym[k][iside][i][j] / (double)(TRAN_TKspace_grid2 * TRAN_TKspace_grid3);
	  }

          MPI_Allreduce(MPI_IN_PLACE, JLocSym[k][iside][i], NUM_c, MPI_DOUBLE_PRECISION, MPI_SUM, comm1);
      ...
      for (iside = 0; iside < 2; iside++){
        for (i = 0; i < NUM_c; i++){
          ...
          for (j = 0; j < NUM_c; j++){
            Jmat[k][iside][i][j] = Jmat[k][iside][i][j] / (double)(TRAN_TKspace_grid2 * TRAN_TKspace_grid3);
	  }

          MPI_Allreduce(MPI_IN_PLACE, Jmat[k][iside][i], NUM_c, MPI_DOUBLE_PRECISION, MPI_SUM, comm1);
      ...
      for (i = 0; i < NUM_c; i++){
        for (j = 0; j < NUM_c; j++) {
          ...
          JLocASym[k][i][j] = JLocASym[k][i][j] / (double)(TRAN_TKspace_grid2 * TRAN_TKspace_grid3);
          RhoNL[k][i][j] = RhoNL[k][i][j] / (double)(TRAN_TKspace_grid2 * TRAN_TKspace_grid3);

        } /* for (j = 0; j < NUM_c; j++) */

        MPI_Allreduce(MPI_IN_PLACE, JLocASym[k][i], NUM_c, MPI_DOUBLE_PRECISION, MPI_SUM, comm1);
        MPI_Allreduce(MPI_IN_PLACE, RhoNL[k][i], NUM_c, MPI_DOUBLE_PRECISION, MPI_SUM, comm1);
  ...
  if (TRAN_Channel == 1){
    ...
    for (kloop = 0; kloop < TRAN_Channel_Nkpoint; kloop++){
      for (iw = 0; iw < TRAN_Channel_Nenergy; iw++){
        ...
        MPI_Allreduce(MPI_IN_PLACE, eigentrans_sum[kloop][iw], SpinP_switch + 1, MPI_DOUBLE_PRECISION, MPI_SUM, comm1);

        for (k = 0; k < SpinP_switch + 1; k++){

          MPI_Allreduce(MPI_IN_PLACE, eigentrans[kloop][iw][k], TRAN_Channel_Num, MPI_DOUBLE_PRECISION, MPI_SUM, comm1);

          for (i = 0; i < TRAN_Channel_Num; i++){

            MPI_Allreduce(MPI_IN_PLACE, EChannel[kloop][iw][k][i], NUM_c, MPI_DOUBLE_COMPLEX, MPI_SUM, comm1);
...
void MTRAN_Read_Tran_HS(MPI_Comm comm1, char *filepath, char *filename, char *ext ) 
{
  ...
  **********************************************/
  ...
  if (TRAN_region[1]==6) periodic_flag = 1;
  else                   periodic_flag = 0; 
		 ...
		 )
{
  ...
  *TRAN_TKspace_grid3 = i_vec[1];
  ...
  /* correct Chemical potentials */

  input_logical("NEGF.bias.apply",&tran_bias_apply,1); /* default=on */

  if (tran_bias_apply==1 && periodic_flag==1){

    double tmp;

    input_double("NEGF.bias.voltage", &tmp, 0.0);  /* in eV */
    tran_biasvoltage_e[0] =-0.5*tmp/eV2Hartree; 
    tran_biasvoltage_e[1] = 0.5*tmp/eV2Hartree; 

    ChemP_e[0] = ChemP_e[0] + tran_biasvoltage_e[0];
    ChemP_e[1] = ChemP_e[1] + tran_biasvoltage_e[1];

    if (myid==0){
      printf("\n");fflush(stdout);
      printf("Chemical potentials were corrected by applying the bias voltage of %6.3f (eV)\n",tmp);  fflush(stdout);
      printf("  Left lead:  %15.12f (eV)\n",ChemP_e[0]*eV2Hartree);fflush(stdout);
      printf("  Right lead: %15.12f (eV)\n\n",ChemP_e[1]*eV2Hartree);fflush(stdout);
    }
  }
                     ...
                     char *tranbfilename)
{
  ...
  /* specify a tranb file */

  input_string("NEGF.tranb.file",tranbfilename,filename);
	  ...
	  for (j=0; j<tnoB; j++){
	    ...
	    for (k=0; k<=SpinP_switch_e[iside]; k++ ){
	      ...
	      H00_e[iside][k][S00_ref(Anum+i,Bnum+j)].i += si*H_e[iside][k][GA_AN][LB_AN][i][j];
            ...
            if (tran_bias_apply==1 && periodic_flag==1 ){
	      for (k=0; k<=SpinP_switch_e[iside]; k++ ){
		H00_e[iside][k][S00_ref(Anum+i,Bnum+j)].r += co*OLP_e[iside][0][GA_AN][LB_AN][i][j]*tran_biasvoltage_e[iside];
		H00_e[iside][k][S00_ref(Anum+i,Bnum+j)].i += si*OLP_e[iside][0][GA_AN][LB_AN][i][j]*tran_biasvoltage_e[iside];
	      }
	    }
	  ...
	  for (j=0; j<tnoB; j++){
	    ...
	    for (k=0; k<=SpinP_switch_e[iside]; k++ ){
	      ...
	      H01_e[iside][k][S00_ref(Anum+i,Bnum+j)].i += si*H_e[iside][k][GA_AN][LB_AN][i][j];
            ...
            if (tran_bias_apply==1 && periodic_flag==1 ){
	      for (k=0; k<=SpinP_switch_e[iside]; k++ ){
		H01_e[iside][k][S00_ref(Anum+i,Bnum+j)].r += co*OLP_e[iside][0][GA_AN][LB_AN][i][j]*tran_biasvoltage_e[iside];
		H01_e[iside][k][S00_ref(Anum+i,Bnum+j)].i += si*OLP_e[iside][0][GA_AN][LB_AN][i][j]*tran_biasvoltage_e[iside];
	      }
	    }
