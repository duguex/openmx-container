...
double func_HG_eta(int p, double og, double gap);
...
int main(int argc, char *argv[]) 
{
  ...
  *******************************************************/
  ...
  else if (job_id==8) pth_Harmonic_Generation_sigma(2);
...
void SHG()
{
  ...
  Ng = 100;
  omega_min = 0.0/eV2Hartree;               
  omega_max = 6.0/eV2Hartree;   
...
void pth_Harmonic_Generation_chi(int pth)
{
  ...
  double omega0,pomega0,omega_min,omega_max,domega,em,en,fm,fn;
  double Beta,x,tmp1,tmp2,tmp3,vtmp[4],Cell_Volume;
  int **omg_range;
  ...
  *************************************************************/
  ...
  omg_range = (int**)malloc(sizeof(int*)*(SpinP_switch+1));
  for (spin=0; spin<(SpinP_switch+1); spin++){
    omg_range[spin] = (int*)malloc(sizeof(int)*(pth+1));
  }
  ...
  *************************************************************/
  ...
  /*************************************************************
        determine the range of omega0 to be calculated
  *************************************************************/
  ...
  */
  ...
  for (omg=(Ng-1); 0<=omg; omg--){
    omega0 = omega_min + (double)omg*domega;

    for (spin=0; spin<=SpinP_switch; spin++){ 
      for (p=1; p<=pth; p++){
      
	if ( ((double)p*omega0)<Egap[spin] && omg_range[spin][p]==0){
          omg_range[spin][p] = omg;
	} 
      }
    }
  } 
	...
	for (q=0; q<independent_count[p]; q++){
	  ...
	  for (n1=0; n1<=pmax_d; n1++){
		...
		for (n=0; n<MatDim; n++){
		  ...
		  //ctmp1.i = -(10.0*exp(-1000.0*sqrt((em-en)*(em-en))) + 0.05/eV2Hartree);
		  ...
		  if (fabs(em-en)<1.0e-2){ 
		    D[p][q][n1][n*MatDim+m] = Complex(0.0,0.0);
		  }
            ...
            for (m=0; m<MatDim; m++){
	      D[p][q][n1][m*MatDim+m] = Complex(0.0,0.0);
	    }
      ...
      for (m=0; m<5; m++){
	for (n=0; n<5; n++){
          printf("m=%2d n=%2d %15.12f %15.12f\n",
                  m,n,D[2][0][0][n*MatDim+m].r,D[2][0][0][n*MatDim+m].i);
	} 
      } 
  MPI_Finalize();
  exit(0);
	...
	********************************************************/
		...
		zgemm_( "N","N", &m, &n, &k, &alpha, P[xyz], &m, 
                ...
                beta = Complex(1.0,0.0); 
		m = MatDim; n = MatDim; k = MatDim;
		zgemm_( "N","N", &m, &n, &k, &alpha, rho[p-1][l], 
                        &m, P[xyz], &k, &beta, work_array, &m );
                  
	      } // end of if (l!=-1)
	    } // xyz

            /* Hadamard product: work_array with 1/(hbar*Omega-(emu-enu)) */
	    ...
	    for (m=0; m<MatDim; m++){
	      ...
	      for (n=0; n<MatDim; n++){
	        ...
	        //ctmp1.i = 10.0*exp(-1000.0*sqrt((em-en)*(em-en))) + 0.05/eV2Hartree;
                ...
                if (m==n){
                  rho[p][q][n*MatDim+m] = Complex(0.0,0.0);
		}
...
void pth_Harmonic_Generation_sigma(int pth)
{
  ...
  double k1,k2,k3,*eval,eta,max_x=30.0,Bi,alphai,scissor;
  double omega0,pomega0,omega_min,omega_max,domega,em,en,fm,fn;
  double Beta,x,tmp1,vtmp[4],Cell_Volume,f;
	  ...
	  for (n1=0; n1<=pth; n1++){
		...
		for (n=0; n<MatDim; n++){
		  ...
		  ctmp1.i =-eta;
                  ...
                  //ctmp1.i = -func_HG_eta(p, double og, double gap);
...
{
  double f;
  f = 1.0/(1.0+exp(-10.0*((double)p*og-gap))) + 1.0e-5;
  return f;
}
	    ...
	    for (j=0; j<tnoB; j++){
	      ...
	      /* store the result to A11, A12, A21, or A22 */
              ...
              if (m<No[spin] && n<No[spin]){
                ...
                A22[0][0][(n-No[spin])*Nuo[spin]+m-No[spin]].r += ctmp2.r*(sx - 0.0*Rx*s);
                A22[0][1][(n-No[spin])*Nuo[spin]+m-No[spin]].r += ctmp2.r*(sy - 0.0*Ry*s);
                A22[0][2][(n-No[spin])*Nuo[spin]+m-No[spin]].r += ctmp2.r*(sz - 0.0*Rz*s);

                A22[0][0][(n-No[spin])*Nuo[spin]+m-No[spin]].i += ctmp2.i*(sx - 0.0*Rx*s);
                A22[0][1][(n-No[spin])*Nuo[spin]+m-No[spin]].i += ctmp2.i*(sy - 0.0*Ry*s);
                A22[0][2][(n-No[spin])*Nuo[spin]+m-No[spin]].i += ctmp2.i*(sz - 0.0*Rz*s);
	      }
